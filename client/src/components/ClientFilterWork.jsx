...
const ClientFilterWork = () => {
  ...
  useEffect(() => {
    async function fetchAppliedUsers() {
      ...
      console.log(
        'fetched users:',
        response.data.data.workdetails.applied_status
      );
        ...
        {AppliedUsers.length > 0 ? (
          <ul className="mt-4 grid gap-6 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
            {AppliedUsers.map((user) => (
              <li
                key={user._id}
                className="flex flex-col justify-between rounded-lg border border-gray-200 bg-white p-6 shadow-md transition-shadow hover:shadow-lg"
              >
                <div>
